...
function struct String_Builder String_Builder_init(struct Arena* arena,
		int capacity);
...
function struct String_Builder String_append(struct String_Builder s1,
		struct String s2);

/// Attempts to append a single character into :s.
/// :returns a new String sharing the same :str buffer as :s if :c fits.
///  Otherwise EMPTY_STRING.
function struct String_Builder String_push(struct String_Builder s, char c);

function struct String String_Builder_build(struct String_Builder s);
